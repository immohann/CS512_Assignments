...
#testing model 
def test_model(test_loader,cuda,lenet,c_loss,wtestingepoc,testingepoc,test,dataset,split,batch_size):
...
def main():
    ...
    for i in range(epoch):
        ...
        for iterator, datapoint in enumerate(train_loader):
            ...
            r_loss += loss.item() * train_X.size(0)
            r_right += torch.sum(y_hats == (labels).data)
        ...
        # test model 
        test_model(test_loader,cuda,lenet,c_loss,wtestingepoc,testingepoc,test,dataset,split,batch_size)

        f_wtrainingepoc = open("Adam_files/wordwise_training.txt", "a")
